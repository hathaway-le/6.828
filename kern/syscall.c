...
sys_exofork(void)
{
	...
	if((rc = env_alloc(&e,cur->env_id))!=0)
		return rc;

	e->env_status = ENV_NOT_RUNNABLE;
	e->env_tf = cur->env_tf;
	e->env_tf.tf_regs.reg_eax = 0;//fork只是创建进程控制块，不调度，因此return到trap_dispatch的值还是给父进程的，子进程应该是不走trap_dispatch的，返回值先存起来
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	user_mem_assert(e, tf, sizeof(struct Trapframe), PTE_U);//tf在父进程的栈里面，栈的虚拟地址区间都一样，只要执行这个命令是，子进程的栈已经map过就没有问题
	tf->tf_eflags &= ~FL_IOPL_MASK;//只有fs那个进程可以使能
	tf->tf_eflags |= FL_IF;
	tf->tf_cs |= GD_UT | 3;
	e->env_tf = *tf;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	rc = page_insert(e->env_pgdir,Page,va,perm);
	if(rc != 0)
	{
		page_free(Page);
		return rc;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// LAB 4: Your code here.
	struct Env *s, *d;
	struct PageInfo * Page_s;
	pte_t * p_s;
	int rc;
	if((rc = envid2env(srcenvid,&s,1)) != 0)
		return rc;	
	if((rc = envid2env(dstenvid,&d,1)) != 0)
		return rc;	
	...
	if((uint32_t)dstva >= UTOP || (uint32_t)dstva != ROUNDDOWN((uint32_t)dstva,PGSIZE))
		return -E_INVAL;
	Page_s = page_lookup(s->env_pgdir,srcva,&p_s);
	if(Page_s == NULL)
		return -E_INVAL;
	if(((perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P)) || (perm & (~PTE_SYSCALL)))
		return -E_INVAL;
...
static int sys_eth_tx(void *a1,uint32_t a2)
{
	user_mem_assert(curenv, a1, a2, PTE_U);
